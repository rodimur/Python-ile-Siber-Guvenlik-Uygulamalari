...
for host in nm.all_hosts():
	...
	for port in nm[host]['tcp'].keys():
		...
		if "script" in nm[host]['tcp'][port]:
			#print(nm[host]['tcp'][port]['scprit']['http-auth-finder'])
			paths = re.findall(host+":"+str(port)+"(.+)HTTP: Basic",nm[host]['tcp'][port]['scprit']['http-auth-finder'])
			for path in paths:
				#print(path)
				new_target = {"host":host,"port":port,"path":path.strip()}
...
for host,port,path in targets:
	...
	creeds = re.findall("(.*)- Valid",nm[host]['tcp'][int(port)]['script']['http-brute'])
